...
* **Frontend Developers**: Site engine is maintained as a separate package (`@objectdocs/site`), isolated in `content/.objectdocs`.

### 2. Clean Integration
ObjectDocs is designed to be added to any existing project:
* All documentation lives in `content/` directory
* Site engine is copied to `content/.objectdocs` (gitignored)
